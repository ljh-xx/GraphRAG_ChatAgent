...
class GraphStore:
    ...
    def query_entity_relations(
        ...
        MATCH path=(e:Entity {name: item.matched_entity})-[:RELATED_TO__REL_HOP_PATTERN__]-(neighbor:Entity)
        WITH item, path, relationships(path) AS rels, length(path) AS hop
        UNWIND rels AS r
